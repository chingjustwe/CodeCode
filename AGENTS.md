...
- **Session management** (`src/session/session-manager.ts`): persists every conversation turn to `{CODEDIR}/sessions/<id>.json`. The REPL creates a new session on startup and auto-saves after each user turn and slash command. `/new` creates a fresh session (clearing history), `/list` shows all persisted sessions with metadata. Session files are JSON with `{id, createdAt, updatedAt, title, messages[]}`.
